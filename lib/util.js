    ...
    bot.context.util = {
        ...
        option_keyboard: ($) => {
            return Extra.HTML().markup((m) => m.inlineKeyboard([
                [
                    m.callbackButton(`${$.util.to_user_currencyF($, $.user.placed_money || 0)}`, 'new_option'),
        ...
        btc_currency: ($) => {
            return $.i18n.t('common.currency.BTC').concat("(BTC)");
        },
        ...
        to_usd: ($, amount) => {
            const x = $.currency_rates.USD;
            if($.user.currency === 'USD')
                return (parseFloat(amount/$.currency_rates.USD).toFixed(config.defaults.precision.USD));
            else
                return parseFloat(amount).toFixed(config.defaults.precision.USD);
        ...
        to_user_currencyF: ($, amount) => {
            if($.user.balance_type == 1)
                return `${amount} ${$.util.user_currency($)}`//`${$.util.to_user_currency($, amount)} ${$.util.user_currency($)}`
            else
                return `${amount} ${$.util.virt_currency($)}`
        },
        to_user_currencyBF: ($, amount) => {
            return `${amount} ${$.util.user_currency($)}`//`${$.util.to_user_currency($, amount)} ${$.util.user_currency($)}`
        },
