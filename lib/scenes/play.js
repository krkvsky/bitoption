...
module.exports = (bot) => {
                ...
                if ( countdown > 0 ) {
                    ...
                    const text = render_game_text($, [
                        ...
                        $.i18n.t('play.bid.game_data', {
                            placed_money: $.util.to_user_currencyF($, $.user.placed_money),
                            placed_time: $.user.placed_time,
                        }),
    ...
    scene.action(/^update_money/, async ($, next) => {
        ...
        try {
            ...
            let text;
            const arr = [1, 3, 5, 10, 15, 30, 50, 100];
            const arr2 = [50, 100, 200, 300, 500, 1000];
            const first_placed = placed_money;
            if (action === 'incr') {
                if($.user.currency === 'USD'){
                    if(placed_money > 50) placed_money+=50;
                    else {
                        for (let i = 0; i < arr.length; i++) {
                            if (placed_money < arr[i]) {
                                placed_money = arr[i];
                                break;
                            }
                        }
                    ...
                    if(placed_money > 500) placed_money+=500;
                    else {
                        for (let j = 0; j < arr2.length; j++) {
                            if (placed_money < arr2[j]) {
                                placed_money = arr2[j];
                                break;
                            }
                        }
