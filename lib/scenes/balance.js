...
const main = async ($, next, enter = true) => {
    ...
    const [[deposits]] = await $.db.execute( 'SELECT * FROM deposits WHERE user_id = ?', [ user.id ]);
...
module.exports = (bot) => {
    ...
    scene.action('deposit', async ($, next) => {
        const cid = $.chat.id;
        ...
        return await $.editMessageText($.i18n.t('balance.deposit', {
            user: $.user,
            currency: $.util.user_currency($),
        }), opts).then(next);
    });
