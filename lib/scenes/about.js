...
module.exports = (bot) => {
    ...
    scene.action('statistic', async ($, next) => {
        ...
        return $.editMessageText($.i18n.t('about.statistic', {
            team_count: stat.cnt,
            team_balance_up_sum: $.util.rub_to_user_currencyF($, stat.balance_up_sum_rub || 0),
            team_deposit_sum: $.util.rub_to_user_currencyF($, stat.deposit_sum_rub || 0),
            team_deposit_sum_bitcoin: $.util.to_btcF($, stat.deposit_sum_bitcoin || 0),
        }), opts).then(next);
    });
