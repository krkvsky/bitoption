...
const main = async ($, next, enter = true) => {
    ...
    // ]));
    const text = $.i18n.t('balance.deposit_qiwi', {
...
module.exports = (bot) => {
    const scene = new Scene('deposit_qiwi');
        ...
        try {
            ...
            if (amount) {
                ...
                const [result] = await $.db.query(
                    ...
                    [ { deposit_sum_rub: deposit_sum_rub }, { telegram_id: cid } ]
                );
                ...
                let opts = Extra.HTML().markup((m) => m.inlineKeyboard([
                    [ m.urlButton($.i18n.t('balance.button.deposit_qiwi'), qiwi_url) ],
                    [ m.callbackButton($.i18n.t('button.cancel'), 'cancel') ]
                ]));

                const message = await $.reply($.i18n.t('balance.deposit_qiwi_end', {
