...
class RatesWatcher extends Watcher {
    ...
    async update_rates() {
        ...
        try {
            // let rate_raw = await exchange.getTicker({ currency: 'USD' });
            // let rate_raw = { last: 2587.566 };

            gdax.getProductTicker(async (error, response, data) => {
                ...
                if (Array.isArray( rates ) && !isEmpty(rates)) {
                    ...
                    let rates_map = rates.map( function( rate ) {
