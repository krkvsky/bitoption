...
class QiwiWatcher extends Watcher {
    ...
    async get_transactions(opts) {
        ...
        dom('.reportsLine.status_SUCCESS').each((index, el) => {
            let transaction = parseInt(dom('.DateWithTransaction .transaction', el).text().replace(/[\s]/g, ''));
            let amount = parseFloat(dom('.IncomeWithExpend.income .cash',el).text().replace(/[\s]/g, ''));
            let deposit_id = parseInt(dom('.ProvWithComment .comment',el).text().replace(/[\s]/g, ''));
            let date = dom('.DateWithTransaction .date', el).text().replace(/[\s]/g, '');
            let time = dom('.DateWithTransaction .time', el).text().replace(/[\s]/g, '');
